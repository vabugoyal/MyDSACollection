...
template <class Body>
concept BodyConcept = requires(
    istream& is, 
    ostream& os, typename Body::value_type& v,                           
    typename Body::value_type const& cv) 
{
    { write(os, cv) };
    { read(is, v) };
};
...
void write(ostream& os, message<isRequest, Body, Fields> const& msg) {
    // write_header(os, msg);
    static_assert(is_body<Body>::value, "Body requirements not met");
    Body::write(os, msg);
};
